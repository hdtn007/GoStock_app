...
<a class="item hovercard" data-w="{{$newWidth}}" data-h="{{$newHeight}}" href="{{ url('photo', $image->id ) }}/{{str_slug($image->title)}}" style="{{$background}}">
		...
		<span class="hover-content">
			<span class="text-truncate title-hover-content" title="{{$image->title}}">
				@if ($image->featured == 'yes') <i class="bi bi-award me-1" title="{{trans('misc.featured')}}"></i> {{trans('misc.featured')}}  @endif
				</span>
			...
			<div class="sub-hover d-flex align-items-center">
				...
				<div class="flex-grow-1 ms-3 text-truncate">
					...
					<span class="me-2 d-none d-lg-inline-block"><i class="bi bi-cart"></i> {{Helper::amountFormat($image->price)}}</span>
					@endif
